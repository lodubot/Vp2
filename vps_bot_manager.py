...
class PM2Manager:
    ...
    async def _run_pm2(cmd: List[str], cwd: str = None, timeout: int = 30) -> Tuple[int, str, str]:
        """Run a PM2 command with timeout, return (returncode, stdout, stderr)"""
        try:
    ...
    async def status(service_name: str) -> Optional[Dict]:
        """Return process info dict or None"""
...
class DeploymentEngine:
    ...
    async def deploy_zip(self, user_id: int, file_path: str, bot_type: str) -> Tuple[bool, str, str]:
        """Returns (success, bot_id, error_message)"""
        async with self.semaphore:
            try:
                ...
                # Get bot name from package.json or folder
    ...
    async def deploy_github(self, user_id: int, repo_url: str, branch: str = 'main',
        ...
        """Clone and deploy from GitHub (public or private)"""
        async with self.semaphore:
            try:
    ...
    async def deploy_single_file(self, user_id: int, file_path: str) -> Tuple[bool, str, str]:
        ...
        async with self.semaphore:
            try:
                ...
                # For node, create minimal package.json if not present
                if bot_type == 'nodejs':
                    pkg_path = os.path.join(bot_dir, 'package.json')
    ...
    async def _install_node_deps(self, bot_dir: str):
        """Install Node dependencies using npm (or yarn/pnpm if detected)"""
        pkg_path = os.path.join(bot_dir, 'package.json')
        if not os.path.exists(pkg_path):
            return
    ...
    async def _install_python_deps(self, bot_dir: str):
        ...
        if os.path.exists(pyproject):
            # Try poetry install if available
            proc = await asyncio.create_subprocess_exec(
...
class SecurityManager:
    ...
    def validate_path(bot_dir: str, user_id: int) -> bool:
        """Ensure bot_dir is under user's directory and not traversed"""
        user_dir = os.path.join(HOSTED_BOTS_DIR, str(user_id))
...
class Handlers:
    def __init__(self, db: Database, bot_manager: BotManager, deploy_engine: DeploymentEngine):
        ...
        self.user_data = {}  # Temporary state for conversations
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            f"{S.BOT} **Infinity X – VPS Bot Manager**\n"
            f"Select an option:",
    ...
    async def handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.user_data.get('awaiting_github'):
            ...
            if len(parts) > 1:
                branch = parts[1]
            if len(parts) > 2:
                token = parts[2]  # optional token
            # Validate URL
            if not repo_url.startswith('http'):
                await update.message.reply_text(f"{S.ERROR} Invalid URL.")
                return
    ...
    async def handle_premium_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if not await self._check_access(update):
            return
        user = update.effective_user
        ...
        elif context.user_data.get('awaiting_premium_remove'):
            try:
                target_id = int(text)
    ...
    # Additional admin handlers can be added similarly (users, stats, cleanup, backup, etc.)
    # For brevity, we implement a few placeholders.

    async def admin_users(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
        # Show list of users with ban/unban options (simplified)
    ...
    async def admin_stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
        # Get bot count, user count, etc.
        user_count = len(await self.db.execute("SELECT COUNT(*) FROM users", fetchone=True))
        bot_count = len(await self.db.execute("SELECT COUNT(*) FROM bots", fetchone=True))
        running = len(await self.db.execute("SELECT COUNT(*) FROM bots WHERE status='running'", fetchone=True))
        text = (
            f"📊 **Statistics**\n"
            f"Users: `{user_count}`\n"
    ...
    async def admin_restart_engine(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
        # Gracefully restart bot (will exit and restart via systemd/supervisor)
        await query.edit_message_text("🔄 Restarting engine...")
        # In production, you might send a signal to supervisor. Here we just exit.
        sys.exit(0)

    async def admin_backup(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
        # Create a backup of database and bots
        backup_name = f"backup_{int(time.time())}.tar.gz"
        backup_path = os.path.join(BACKUP_DIR, backup_name)
        # Simple tar: compress DB and bot dirs (but avoid large bots)
        # For demonstration, just copy DB
    ...
    async def terminal_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if cmd == '/exit':
            ...
            return
        # Execute command in bot directory
        try:
...
async def post_init(application: Application):
    # Create database tables (now safe – inside the running event loop)
    db = Database()
...
def main():
    ...
    app.add_handler(CommandHandler("unlock", handlers.toggle_unlock))  # simplified
    ...
    # Premium text input
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handlers.handle_premium_text))
